...
async function initializeOracle(displayTraits, inscriptionId = null) {
  ...
  if (inscriptionId && inscriptionId != "MINT_INSCRIPTION_ID") {
    try {
      inscriptionInfo = await getMetadata(`https://ordinals.com/r/inscription/${inscriptionId}`);
    } catch (error) {
...
rateLimiter.set(key, Date.now());
  ...
  const url = page === 0 
    ? `https://ordinals.com/r/sat/${sat}`
    : `https://ordinals.com/r/sat/${sat}/${page}`;
  try {
...
async function getInscriptionIdAt(sat, index, skip = 0) {
  ...
  const url = `https://ordinals.com/r/sat/${sat}/at/${adjustedIndex}`;
...
async function getCurrentBlockHeight() {
  const url = 'https://ordinals.com/r/blockheight';
  return getMetadata(url);
}    
...
function displayScrollingMessages(messages, interval = 5000, repeat = true, container = document.body, scrollText = true) {
  ...
  //console.log(`Total messages: ${messageElements.size}`);
...
async function processMessages(oracleSats, allMessages = false, displayInterval = 5000, repeat = true, container = document.body, scrollText = true, launchBlock = null, currentBlock = null) {
        ...
        content.messages.forEach(msg => {
          ...
          if (filterArray && msgFilter) { 
            if (Array.isArray(filterArray) && !filter.includes(msgFilter)) {
              //console.log('Filter mismatch:', msgFilter, filterArray, sat, effectiveBlock, effectiveStop);
              return;
            ...
            //console.log('No Filter, msgFilter:', msgFilter, sat, effectiveBlock, effectiveStop);
            return;
          } else if (filterArray && !msgFilter) {
            //console.log('No msgFilter, filter:', filterArray, sat, effectiveBlock, effectiveStop);
            return;
          }

          if (!allMessages && (effectiveBlock > currentHeight || currentHeight > effectiveStop)) {
            //console.log('Message excluded:', msg, effectiveBlock, currentHeight, effectiveStop);
            return;
          }
          //console.log('Message included:', msg, effectiveBlock, currentHeight, effectiveStop);
  ...
  //console.log('Final messageMap:', messageMap);
