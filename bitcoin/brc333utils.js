...
async function getInscriptionsForSat(sat, backup = null, last = false, options = {}) {
    ...
    console.log('Checking sat for inscriptions:', sat);
    if (sat === undefined) {
    ...
    if (last) {
        const url = `https://ordinals.com/r/sat/${sat}/at/-1`;
        try {
        ...
        while (!reachedEnd) {
            ...
            try {
                ...
                if (response && Array.isArray(response.ids)) {
                    ...
                    console.log(`Fetched ${newIds.length} IDs for sat ${sat} on page ${page}, total skipped: ${totalSkipped}, total fetched: ${totalFetched}`);
                    page++;
                } else {
...
async function getInscriptionContent( isSat, field ) {
    ...
    if (isSat) {
        ...
        url = `https://ordinals.com/content/${mostRecent}`;
    } else {
        url = `https://ordinals.com/content/${field}`;
    }
